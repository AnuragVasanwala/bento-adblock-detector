(self.AMP=self.AMP||[]).push({m:0,v:"2110121450050",n:"amp-auto-lightbox",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var Services = /* @__PURE__ */ function() {
    ...
    Services2.ampdoc = function ampdoc2(nodeOrAmpDoc) {
    ...
    Services2.analyticsForDoc = function analyticsForDoc(element, loadAnalytics) {
      ...
      if (loadAnalytics) {
        var ampdoc2 = getAmpdoc(element);
        Services2.extensionsFor(ampdoc2.win).installExtensionForDoc(ampdoc2, "amp-analytics");
      }
  ...
  var Criteria = /* @__PURE__ */ function() {
    ...
    Criteria2.meetsAll = function meetsAll(element, ampdoc2, renderWidth, renderHeight) {
      return Criteria2.meetsSizingCriteria(element, ampdoc2, renderWidth, renderHeight) && Criteria2.meetsTreeShapeCriteria(element, ampdoc2);
    };
    Criteria2.meetsTreeShapeCriteria = function meetsTreeShapeCriteria(element, ampdoc2) {
      ...
      var actions = Services.actionServiceForDoc(ampdoc2 || element);
      return !actions.hasResolvableAction(element, "tap");
    };
    Criteria2.meetsSizingCriteria = function meetsSizingCriteria(element, ampdoc2, renderWidth, renderHeight) {
  ...
  var DocMetaAnnotations = /* @__PURE__ */ function() {
    ...
    DocMetaAnnotations2.getOgType = function getOgType(ampdoc2) {
      var tag = ampdoc2.getRootNode().querySelector(META_OG_TYPE);
      if (tag) {
        return tag.getAttribute("content");
      }
    };
    DocMetaAnnotations2.hasValidOgType = function hasValidOgType(ampdoc2) {
      return DocMetaAnnotations2.getOgType(ampdoc2) == ENABLED_OG_TYPE_ARTICLE;
    };
    DocMetaAnnotations2.getAllLdJsonTypes = function getAllLdJsonTypes(ampdoc2) {
      return toArray(ampdoc2.getRootNode().querySelectorAll(SCRIPT_LD_JSON)).map(function(el) {
        var textContent = el.textContent;
        return (tryParseJson(textContent) || {})["@type"];
      }).filter(Boolean);
    };
    DocMetaAnnotations2.hasValidLdJsonType = function hasValidLdJsonType(ampdoc2) {
      return DocMetaAnnotations2.getAllLdJsonTypes(ampdoc2).some(function(type) {
        return ENABLED_LD_JSON_TYPES[type];
      });
    };
    return DocMetaAnnotations2;
  }();
  function usesLightboxExplicitly(ampdoc2) {
    ...
    var exists = function exists2(selector) {
      return !!ampdoc2.getRootNode().querySelector(selector);
    };
    return exists(requiredExtensionSelector) && exists(lightboxedElementsSelector);
  }
  function isEnabledForDoc(ampdoc2) {
    if (usesLightboxExplicitly(ampdoc2)) {
      return false;
    }
    return DocMetaAnnotations.hasValidOgType(ampdoc2) || DocMetaAnnotations.hasValidLdJsonType(ampdoc2);
  }
  var uid = 0;
  function generateLightboxUid() {
    return "i-amphtml-auto-lightbox-" + uid++;
  }
  function apply(ampdoc2, element) {
    var mutator = Services.mutatorForDoc(ampdoc2);
    ...
    return mutatePromise.then(function() {
      Services.extensionsFor(ampdoc2.win).installExtensionForDoc(ampdoc2, REQUIRED_EXTENSION);
      dispatchCustomEvent(element, AutoLightboxEvents.NEWLY_SET);
      return element;
    });
  }
  function runCandidates(ampdoc2, candidates) {
    return candidates.map(function(candidate) {
      return whenLoaded(candidate).then(function() {
        return measureIntersectionNoRoot(candidate).then(function(_ref) {
          ...
          if (!Criteria.meetsAll(candidate, ampdoc2, width, height)) {
            return;
          }
          dev().info(TAG, "apply", candidate);
          return apply(ampdoc2, candidate);
        });
      }, NOOP);
    });
  }
  function scan(ampdoc2, opt_root) {
    if (!isEnabledForDoc(ampdoc2)) {
      dev().info(TAG, "disabled");
      return;
    }
    var root = opt_root || ampdoc2.win.document;
    return runCandidates(ampdoc2, Scanner.getCandidates(root));
  }
  var _AMP = AMP;
  var ampdoc = _AMP.ampdoc;
