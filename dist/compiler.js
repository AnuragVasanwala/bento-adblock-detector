(() => {
  ...
  function fromNode(node) {
    ...
    const attributes = Array.from(elementNode.attributes).map(({ name, value }) => ({ name, value }));
    return {
  ...
  var DOMTokenList = class {
    ...
    add(...tokens) {
      const oldValue = this.value;
      this[43].splice(0, this[43].length, ...new Set(this[43].concat(tokens)));
      this[67](oldValue, this.value);
    }
    remove(...tokens) {
      const oldValue = this.value;
      this[43].splice(0, this[43].length, ...new Set(this[43].filter((token) => !tokens.includes(token))));
      this[67](oldValue, this.value);
    }
    replace(token, newToken) {
      ...
      this[43].splice(0, this[43].length, ...set);
      this[67](oldValue, this.value);
    }
    toggle(token, force) {
  ...
  function fromTreeProto(ast2) {
    const doc = createDocument();
    Object.defineProperties(doc, {
      compatMode: {
        value: ast2.quirks_mode ? "BackCompat" : "CSS1Compat"
      }
    });
    const firstNodeTagId = ast2.tree?.[0]?.tagid;
    if (firstNodeTagId !== getTagId("ZERO_LENGTH")) {
      throw new Error(`HTML must begin with a #document tag, found: ${firstNodeTagId}`);
    }
    fromTreeProtoHelper(ast2.tree[0].children, doc, doc);
    return doc;
  }
  ...
  function renderNodeDeep(node, instructions, { handleError = defaultHandleError } = {}) {
    if (node.tagName.toLowerCase() in instructions) {
      const buildDom3 = instructions[node.tagName.toLowerCase()];
      buildDom3(node);
    }
    for (let [tagName, buildDom3] of Object.entries(instructions)) {
  ...
  function compile(request) {
    ...
    if (document) {
      return {
        document: renderAstDocument(document, getBuilders(versions))
      };
    }
    return { nodes: renderAstNodes(nodes, getBuilders(versions)) };
  }
